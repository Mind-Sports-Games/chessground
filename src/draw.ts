...
import { unselect, cancelMove } from './board';
...
export function processDraw(state: State): void {
  requestAnimationFrame(() => {
    ...
    if (cur) {
      const keyAtDomPos = state.getKeyAtDomPos(
        cur.pos,
        state.orientation,
        state.dom.bounds(),
        state.dimensions,
      ...
      const mouseSq = cur.snapToValidMove
        ? state.getSnappedKeyAtDomPos(cur.orig, cur.pos, state.orientation, state.dom.bounds(), state.dimensions, state.variant)
        : keyAtDomPos;
