...
export function start(state: State, e: cg.MouchEvent): void {
  ...
  const pos = eventPosition(e)!,
    orig = getKeyAtDomPos(pos, state.orientation, whitePov(state), state.dom.bounds(), state.geometry);
  if (!orig) return;
...
export function processDraw(state: State): void {
  requestAnimationFrame(() => {
    ...
    if (cur) {
      const keyAtDomPos = getKeyAtDomPos(cur.pos, state.orientation, whitePov(state), state.dom.bounds(), state.geometry);
      if (!keyAtDomPos) {
        cur.snapToValidMove = false;
      }
      const mouseSq = cur.snapToValidMove
        ? getSnappedKeyAtDomPos(cur.orig, cur.pos, state.orientation, state.dom.bounds(), state.geometry)
        : keyAtDomPos;
